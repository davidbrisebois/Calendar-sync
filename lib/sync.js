...
function findUrls(text) {
  ...
  const matches = src.match(/https?:\/\/[^\s<>"]+/gi) || [];
  return matches;
}

function buildBodyContent(event) {
  const description = String(event.description || "");
  const lines = description.split(/\r?\n/).map((line) => escapeHtml(line));
  let html = lines.join("<br>");

  const urls = new Set([
    ...findUrls(description),
    ...findUrls(event.location || ""),
  ...
  for (const url of urls) {
    ...
    if (html.includes(safeUrl)) {
      html = html.replaceAll(safeUrl, anchor);
    } else {
      html += `${html ? "<br><br>" : ""}${anchor}`;
    }
  }
...
function toSignature(event) {
  ...
  return [
    ...
    Array.from(exdateDateSet(event)).sort().join(","),
  ].join("|");
}
...
function exdateValues(event) {
  ...
  if (typeof event.exdate === "object") {
    return Object.values(event.exdate).map((value) => {
      if (value instanceof Date) return value;
      if (value?.val instanceof Date) return value.val;
...
async function applyExdateExclusions(accessToken, graphEventId, event) {
  const exDates = Array.from(exdateDateSet(event));
  if (!exDates.length) return;

  const sorted = [...exDates].sort();
  const start = `${sorted[0]}T00:00:00Z`;
  const end = `${sorted[sorted.length - 1]}T23:59:59Z`;

  const instancesRes = await graphRequest(
    accessToken,
    "GET",
    `/me/events/${encodeURIComponent(graphEventId)}/instances`,
    undefined,
    {
      startDateTime: start,
      endDateTime: end,
  ...
  const instances = instancesRes.data?.value || [];

  for (const instance of instances) {
    const startDate = instance?.start?.dateTime ? toIsoDate(instance.start.dateTime) : null;
    if (!startDate || !exDates.includes(startDate)) continue;

    try {
      await graphRequest(accessToken, "DELETE", `/me/events/${encodeURIComponent(instance.id)}`);
    } catch (err) {
...
function toGraphPayload(config, event) {
  ...
  return {
    ...
    body: {
      contentType: "HTML",
      content: buildBodyContent(event),
    },
    location: {
...
export async function syncUser(config) {
  ...
  for (const event of icsEvents) {
    ...
    if (existingMapping) {
      ...
      try {
        ...
        await applyExdateExclusions(accessToken, existingMapping.graphEventId, event);
        stats.updated += 1;
        continue;
      } catch (err) {
        if (err?.response?.status !== 404) {
    ...
    if (fallback?.id) {
      ...
      await applyExdateExclusions(accessToken, fallback.id, event);
      stats.unchanged += 1;
      continue;
    }
    ...
    await applyExdateExclusions(accessToken, created.data.id, event);
    stats.created += 1;
  }
  ...
  return {
    ...stats,
    processed: stats.created + stats.updated + stats.unchanged,
  };
}
